...
\svgpath{{../problem/data/athletes/}{../answer/answer_graph}}
...
\begin{center}
    \resizebox{0.8\textwidth}{!}{
        \begin{tabular}{clc}
            ...
            $C_w$         & drag coefficient                                        & Dimensionless \\
...
\begin{equation}
    ...
    \begin{array}{c}
        ...
        F_s=mg\sin \theta                 \\
        \theta=mg\arctan\frac{d h}{d t}   \\%角度算法
...
\end{equation}
 ...
 \end{figure}
Then, for we can set $R$ to any proper number, here we set it to 1000. So we get a file of the riders.%此处应有表
Our model has the following advantages:
\begin{enumerate}
    \item The conversion efficiency and battery cost are considered together.%应用了动态规划，能保证得到全局最优解
    \item High power electrical appliances such as hairdryers are considered, so the stability is high.%计算效率高
...
\begin{enumerate}
    \item No account is taken of battery size and weight.%忽略了运动员之间的影响
    \item No account is taken of different types of lithium battery life.%规划策略可能不易实行
    \item No account is taken of the change in the number of replaceable batteries caused by different seasons%没有考虑转弯
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \subcaptionbox*{men elite route map (from https://www.flanders2021.com)}{\includegraphics[width=0.4\columnwidth]{men-elite-individual-time-trial-map}}
